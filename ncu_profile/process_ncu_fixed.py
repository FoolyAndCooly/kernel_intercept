...
kernels = []
metrics_to_get = ['Duration', 'Block Size', 'Grid Size', 'Compute (SM) [%]', 'Compute (SM) Throughput', 'DRAM Throughput', 'Registers Per Thread', 'Static Shared Memory Per Block']
...
for kernel in kernels:
    ...
    if len(kernel) < 4:
        kernel += [0] * (4 - len(kernel))
    num_threads = parse_num(kernel[-2]) * parse_num(kernel[-3])
    num_registers = num_threads * parse_num(kernel[-1])
